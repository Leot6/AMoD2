...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        print("[ERROR] Wrong argument aside from the program name for correct execution! \n"
              ...
              "- Example: python3 {} \n"
              "       or: python3 {} \"./config/platform_demo.yml\" \"./media-gitignore/manhattan.png\"\n".format(
            sys.argv[0], sys.argv[0]))
        sys.exit(1)
    else:
        config_file_local_path = sys.argv[1]
        map_image_file_local_path = sys.argv[2]

    if config_file_local_path.startswith('.'):
        config_file_local_path = config_file_local_path[1:]
    if map_image_file_local_path.startswith('.'):
        map_image_file_local_path = map_image_file_local_path[1:]

    config_file_path = root_path + config_file_local_path
    print("Loading the boundary of the simulated area from config at {}.".format(config_file_path))

    # Load the boundary of the area.
    lon_min, lon_max, lat_min, lat_max = get_boundary_from_config(config_file_path)
    ...
    map_image_file_path = root_path + map_image_file_local_path
    img.save(map_image_file_path)

    print("[INFO] ({}s) Image written to path {}.".format(round(time.time()-stime, 2), map_image_file_path))
